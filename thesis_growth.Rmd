...
```{r}
M.lm <- gls(ANN_INC ~ TREATMENT + SPECIES, data = growth)
plot(resid(M.lm) ~ growth$TREATMENT)
plot(resid(M.lm) ~ growth$SPECIES)
summary(M.lm)
```

```{r}
vf2 <- varIdent(form= ~ 1 | TREATMENT * SPECIES)
M.identvar <- gls(ANN_INC ~ TREATMENT + SPECIES, data = growth, weights = vf2)
summary(M.identvar)
```
...
```{r}
M.lm2 <- lm(data = growth, RGR ~ TREATMENT + SPECIES + COMP + TREATMENT:SPECIES)
summary(M.lm2)
```
