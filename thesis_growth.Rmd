...
plot.design(ANN_INC ~ TRT + SPP + COMP, data = growth_out)
with(growth_out, {
  interaction.plot(x.factor = TRT, trace.factor = SPP, response = ANN_INC, col = SPP, type = "l")
  interaction.plot(x.factor = SPP, trace.factor = TRT, response = ANN_INC)
  })
```
...
ggplot() + 
  geom_point(aes(x = TRT, y = mean, group = SPP, col = SPP)) + 
  geom_line(aes(x = TRT, y = mean, group = SPP, col = SPP))
```
