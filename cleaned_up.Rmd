...
```{r}
SBAI_om <- omega_sq(SBAI_aov, ci.lvl = 0.95)

TBAI_om <- omega_sq(TBAI_aov, ci.lvl = 0.95)

CIs <- data.frame(metric = c('SBAI', 'TBAI'), 
                  omegasq = c(SBAI_om[[2]], TBAI_om[[2]]), 
                  low = c(SBAI_om[[3]], TBAI_om[[3]]), 
                  high = c(SBAI_om[[4]], TBAI_om[[4]]))

ggplot(CIs) + 
  geom_point(aes(x = metric, y = omegasq)) + 
  geom_errorbar(aes(x = metric, ymin = low, ymax = high), width = 0.1) + 
  ylim(-1,1) + 
  labs(x = 'Growth metric', 
       y = 'Omega squared')
```
...
TBAI_CI <- data.frame(TRT = c('Burn only', 'Mastication + Burn', 'Mastication + herbicide'), 
...
ggplot(TBAI_CI) + 
  ...
  labs(x = 'Treatment', 
       y = 'Effect size (raw mean difference)')
```

```{r}
SBAI_bounds <- TukeyHSD(SBAI_aov)

SBAI_CI <- data.frame(TRT = c('Burn only', 'Mastication + Burn', 'Mastication + herbicide'), 
